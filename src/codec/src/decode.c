...
static errorCode decode(EXIPSchema *schemaPtr, unsigned char outFlag, void *in_stream, boolean outOfBandOpts, EXIOptions *opts, size_t (*inputStream)(void *buf, size_t size, void *stream))
{
	Parser testParser;
...
{
	return decode(schemaPtr, outFlag, in_stream, outOfBandOpts, opts, readFileInputStream);
}
...
{
	return decode(schemaPtr, outFlag, in_stream, outOfBandOpts, opts, readBufferInputStream);
}
